...
class GameState:
    def __init__(self, game_map, player_direction=Direction.RIGHT.value, is_picked_key=False, goal_name=None,
                 goal_location=None, visited_locations=[]):
    ...
    def is_goal_state(self) -> bool:
        return self._player_location == self._goal_location

    def is_picked_key(self):
        return self._is_picked_key
    ...
    @staticmethod
    def _find_next_target(game_map: List[List[str]], player_location, is_picked_key,
                          visited_locations: List[Tuple[int, int]]) -> tuple:
        """
        ...
        while queue:
            ...
            for dr, dc in directions:
                ...
                if 0 <= nr < rows and 0 <= nc < cols and game_map[nr][nc] not in ['wall', 'lava'] and (
    ...
    def __hash__(self):
        ...
        game_map_hashable = tuple(tuple(row) for row in self._game_map)

        # Create a tuple of all the relevant attributes for hashing
        return hash((game_map_hashable,
                     self._player_direction,
    ...
    def __eq__(self, other):
        # Ensure equality is based on the same fields used for hashing
        if isinstance(other, GameState):
            return (self._game_map == other._game_map and
                    self._player_direction == other._player_direction and
                    self._player_location == other._player_location and
                    self._is_picked_key == other._is_picked_key and
                    self._goal_location == other._goal_location and
                    self._goal_name == other._goal_name)
        return False
